...
pdf("pca_chart.pdf",width=5,height=5)
...
pdf("cell_heatmaps.pdf",height=7,width=7)
heatmap.2( dec2, col=colfunc(25),scale="row",
 trace="none",margins = c(5,5), cexRow=.7, cexCol=.8,  main="cell type abundances")
heatmap.2( dec2, col=colfunc(25),scale="none",
 trace="none",margins = c(5,5), cexRow=.7, cexCol=.8,  main="cell type abundances")
...
pdf("cell_bars.pdf",height=5,width=5)
barplot(apply(dec2,1,sd),horiz=TRUE,las=1,xlab="SD of cell proportions (%)")
dev.off()
...
null <- lapply(1:17 , function(i) {
  cellname <- colnames(sscell_t0)[i]
  t0vals_dex <- subset(sscell_t0,trt==0)[,i]
  t0vals_placebo <- subset(sscell_t0,trt==1)[,i]

  eosvals_dex <- subset(sscell_eos,trt==0)[,i]
  eosvals_placebo <- subset(sscell_eos,trt==1)[,i]

  pod1vals_dex <- subset(sscell_pod1,trt==0)[,i]
  pod1vals_placebo <- subset(sscell_pod1,trt==1)[,i]

  cl <- list("t0 placebo"=t0vals_placebo,"t0 dex"=t0vals_dex,
    "EOS placebo"=eosvals_placebo, "EOS dex"=eosvals_dex,
    "POD1 placebo"=pod1vals_placebo, "POD1 dex"=pod1vals_dex )
  boxplot(cl,col="white",cex=0,ylab="estimated relative cell proportion",main=cellname,las=2)
  beeswarm(cl,pch=1,cex=0.8,add=TRUE)
})

pdf("cell_boxplots.pdf",height=5,width=5)
null <- lapply(1:17 , function(i) {
  cellname <- colnames(sscell_t0)[i]
  t0vals_dex <- subset(sscell_t0,trt==0)[,i]
  t0vals_placebo <- subset(sscell_t0,trt==1)[,i]
...
sscell_t0$crp <- ss[match(rownames(sscell_t0),rownames(ss)),"crp_group"]
sscell_eos$crp <- ss[match(rownames(sscell_eos),rownames(ss)),"crp_group"]
sscell_pod1$crp <- ss[match(rownames(sscell_pod1),rownames(ss)),"crp_group"]

null <- lapply(1:17 , function(i) {
  cellname <- colnames(sscell_t0)[i]
  t0vals_lo <- subset(sscell_t0,crp==1)[,i]
  t0vals_hi <- subset(sscell_t0,crp==4)[,i]

  eosvals_lo <- subset(sscell_eos,crp==1)[,i]
  eosvals_hi <- subset(sscell_eos,crp==4)[,i]

  pod1vals_lo <- subset(sscell_pod1,crp==1)[,i]
  pod1vals_hi <- subset(sscell_pod1,crp==4)[,i]

  cl <- list("t0 low"=t0vals_lo,"t0 high"=t0vals_hi,
    "EOS low"=eosvals_lo, "EOS high"=eosvals_hi,
    "POD1 low"=pod1vals_lo, "POD1 high"=pod1vals_hi )
  boxplot(cl,col="white",cex=0,ylab="estimated relative cell proportion",main=cellname,las=2)
  beeswarm(cl,pch=1,cex=0.8,add=TRUE)
})
...
null <- lapply(1:17 , function(i) {
  cellname <- colnames(sscell_t0)[i]
  t0vals_lo_placebo <- subset(sscell_t0,crp==1 & trt==0 )[,i]
  t0vals_lo_dex <- subset(sscell_t0,crp==4 & trt==1)[,i]
  t0vals_hi_placebo <- subset(sscell_t0,crp==4 & trt==0 )[,i]
  t0vals_hi_dex <- subset(sscell_t0,crp==4 & trt==1 )[,i]

  eosvals_lo_placebo <- subset(sscell_eos,crp==1 & trt==0 )[,i]
  eosvals_lo_dex <- subset(sscell_eos,crp==1 & trt==1 )[,i]
  eosvals_hi_placebo <- subset(sscell_eos,crp==4 & trt==0 )[,i]
  eosvals_hi_dex <- subset(sscell_eos,crp==4 & trt==1 )[,i]

  pod1vals_lo_placebo <- subset(sscell_pod1,crp==1 & trt==0)[,i]
  pod1vals_lo_dex <- subset(sscell_pod1,crp==1 & trt==1)[,i]
  pod1vals_hi_placebo <- subset(sscell_pod1,crp==4 & trt==0)[,i]
  pod1vals_hi_dex <- subset(sscell_pod1,crp==4 & trt==1)[,i]

  cl <- list("t0 low placebo"=t0vals_lo_placebo, "t0 low dex"=t0vals_lo_dex,
    "t0 high placebo"=t0vals_hi_placebo,"t0 high dex"=t0vals_hi_dex,
    "EOS low placebo"=eosvals_lo_placebo, "EOS low dex"=eosvals_lo_dex,
    "EOS high placebo"=eosvals_hi_placebo,"EOS high dex"=eosvals_hi_dex,
    "POD1 low placebo"=pod1vals_lo_placebo, "POD1 low dex"=pod1vals_lo_dex,
    "POD1 high placebo"=pod1vals_hi_placebo,"POD1 high dex"=pod1vals_hi_dex)
...
Placebo is the control group and dex is the case.
Previously this was blinded as treatment group A (dex) and group B (placebo).
...
Save DEX responsive genes.
We will quantify DEX response then investigate patterns in t0 that associate with this reponse.
...
trt2_dex_response_df <- trt2_dex_response_df[which(rownames(trt2_dex_response_df) %in% rownames(ss)),,drop=FALSE]
mx <- xt0f
ss2 <- as.data.frame(cbind(ss_t0,sscell_t0))
...
names(trt2_dex_response_scaled) <- gsub("EOS","T0",names(trt2_dex_response_scaled))

trt2_dex_response_scaled <- trt2_dex_response_scaled[which(names(trt2_dex_response_scaled) %in% colnames(mx))]
...
dge <- as.data.frame(zz[order(zz$pvalue),])
head(dge[order(dge$pvalue),1:6],10)
mean(abs(dge$stat[which(!is.na(dge$stat))]))

```

Now look at the top genes. Correlation between log transformed expression vs scaled dex response.
...
cor_res <- lapply(1:nrow(rpm), function(i) {
  unlist(cor.test(log10(rpm[i,]),trt2_dex_response_scaled)[c(3,4)])
})
...
lapply(1:10,function(i) {
  g <- rownames(cor_df)[i]
  plot(log10(rpm[grep(g,rownames(rpm)),]),trt2_dex_response_scaled,main=g)
})
